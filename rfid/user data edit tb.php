<?php
	...
	if ( !empty($_POST)) {
		...
		$valid = true;
		if (empty($fname)) {
			$fnameError = 'Please enter Family Name';
			$valid = false;
		}

		if (empty($bname)) {
			$bnameError = 'Please enter Main Beneficiary Name';
			$valid = false;
		}
		...
		if (empty($proof)) {
            $proofError = 'Please select Proof';
            $valid = false;
        }
		...
		if ($valid) {
			...
			$sql = "UPDATE tbl_data set fname = ?, bname = ?, mobile =?, email = ?, gender =? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname,$bname,$mobile,$email,$gender,$id));
			Database::disconnect();
			...
			<div class="center" style="margin: 0 auto; width:495px; border-style: solid; border-color: #f2f2f2;">
				...
				<form class="form-horizontal" action="user data edit tb.php?id=<?php echo $id?>" method="post">
					...
					<div class="form-actions">
						<button type="submit" class="btn btn-success" href="user data.php">Update</button>
